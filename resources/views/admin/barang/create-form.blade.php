<section>
    ...
    <form method="post" action="{{ route('barang.store') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
        ...
        <div>
            ...
            <x-text-input id="nama" name="nama" type="text" value="{{ old('nama') }}" class="mt-1 block w-full" />
        ...
        <div>
            ...
            <x-text-area id="deskripsi" name="deskripsi" type="text" class="mt-1 block w-full">{{old('deskripsi')}}</x-text-area>
            <x-input-error :messages="$errors->get('deskripsi')" class="mt-2" />
        </div>

        <div>
            <x-input-label for="stock" :value="__('Stock Barang')" />
            <x-text-input id="stock" name="stock" type="number" min="1" value="{{ old('stock') }}" class="mt-1 block w-full" />
